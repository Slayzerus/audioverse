"""
MoMask text-to-motion sidecar.
Wraps https://github.com/EricGuo5513/momask-codes in a FastAPI service.

Build:
  docker build -t audioverse/motion-momask -f Dockerfile .

Run:
  docker run -p 8302:8302 --gpus all audioverse/motion-momask
"""
...
app = FastAPI(title="MoMask Sidecar", version="1.0.0")
logger = logging.getLogger("momask")
...
def get_model():
    ...
    logger.info("Loading MoMask model...")
    ...
    try:
        ...
        vq_model = RVQVAE(opt).to(opt.device)
        vq_ckpt = torch.load(
            os.environ.get("MOMASK_VQ_CHECKPOINT", "checkpoints/vq/best.tar"),
            map_location=opt.device,
            weights_only=False,
        )
        vq_model.load_state_dict(vq_ckpt["net"], strict=False)
        vq_model.eval()

        t2m_model = MaskTransformer(opt).to(opt.device)
        t2m_ckpt = torch.load(
            os.environ.get("MOMASK_T2M_CHECKPOINT", "checkpoints/t2m/best.tar"),
            map_location=opt.device,
            weights_only=False,
        )
        t2m_model.load_state_dict(t2m_ckpt["trans"], strict=False)
        t2m_model.eval()

        _model = {"vq": vq_model, "t2m": t2m_model, "opt": opt}
        logger.info("MoMask model loaded successfully")
    except ImportError:
        logger.warning("MoMask not installed — running in DEMO mode")
        _model = "demo"
...
@app.get("/health")
def health():
    return {"status": "ok", "engine": "momask"}
...
def _infer(model_data: dict, prompt: str, total_frames: int, fps: float) -> np.ndarray:
    """Run MoMask masked transformer inference."""
    import torch
    ...
    with torch.no_grad():
        token_len = total_frames // 4
        tokens = t2m.generate([prompt], [token_len], temperature=1.0, force_mask=True)
        motion = vq.forward_decoder(tokens)

    if isinstance(motion, torch.Tensor):
        motion = motion.squeeze().cpu().numpy()

    if motion.ndim == 2:
        n_joints = motion.shape[1] // 3
        motion = motion.reshape(-1, n_joints, 3)
...
def _demo_motion(total_frames: int, num_joints: int) -> np.ndarray:
    ...
    for j in range(num_joints):
        phase = j * 0.5
        motion[:, j, 0] = np.sin(t * 1.3 + phase) * 0.35
        motion[:, j, 1] = j * 0.15 + np.cos(t * 0.9 + phase) * 0.06
        motion[:, j, 2] = np.sin(t * 0.5 + phase) * 0.25
    return motion
