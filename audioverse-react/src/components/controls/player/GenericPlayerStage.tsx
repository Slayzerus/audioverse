...
const GenericPlayerStage: React.FC<Props> = ({
    ...
    useEffect(() => {
        ...
        if (source?.kind === "hls" && isM3U8(source.url)) {
            const attachNative = () => {
                ...
                audio.load();
                if (autoPlay) audio.play().catch(() => { /* Expected: autoplay may be blocked by browser */ });
            };
            ...
            (async () => {
                ...
                if (Hls.isSupported()) {
                    ...
                    hls.on(Hls.Events.MANIFEST_PARSED, () => autoPlay && audio.play().catch(() => { /* Expected: autoplay may be blocked by browser */ }));
                } else {
                    attachNative();
                }
            })();
    ...
    // default mode selection (persist in localStorage handled by visualizer)
    ...
    return (
        <div
            ...
            style={{
                width: "100%",
                height,
                background: hasVideo ? "transparent" : "var(--player-bg, #0f172a)",
                borderRadius: 8,
                overflow: "hidden",
                position: "relative",
                display: "grid",
        ...
        >
            ...
            {(source?.kind === "audio" || source?.kind === "hls") && (
                <>
                    {/* placeholder / cover — now covered by visualizer when mode != "video" */}
                    ...
                    <audio
                        ref={audioRef}
                        onEnded={onEnd}
                        onLoadedMetadata={() => {/* duration is caught by hook in the parent component */}}
            ...
            {/* OVERLAY: mode switcher + visualizations + VU meter (persist in localStorage) */}
