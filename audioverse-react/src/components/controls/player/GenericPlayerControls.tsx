    ...
    <>
        ...
        <div className="gp-controls" style={{ display: "grid", gridTemplateColumns: "auto 1fr auto", gap: 12, alignItems: "center" }}>
            <div style={{ display: "flex", gap: 8 }}>
                <button onClick={onPrev} disabled={index <= 0} title="Prev" style={btn}>⏮</button>
                <button onClick={onToggle} title={isPlaying ? "Pause" : "Play"} style={btn}>{isPlaying ? "⏸" : "▶"}</button>
                <button onClick={onNext} disabled={index >= count - 1} title="Next" style={btn}>⏭</button>
            </div>
...
export default GenericPlayerControls;
