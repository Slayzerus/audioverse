...
export default function ReactionPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'reaction')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'elimination', label: t('miniGames.elimination', 'Elimination') },
      { value: 'marathon', label: t('miniGames.marathon', 'Marathon') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'rounds', label: t('miniGames.rounds', 'Rounds'), type: 'range', min: 3, max: 10, step: 1, defaultValue: 5 },
    { key: 'minDelay', label: t('miniGames.minDelay', 'Min Delay (sec)'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 2 },
  ]

  if (phase === 'game') {
    return <ReactionGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.reaction', 'Reaction')}`}
      minPlayers={1}
      maxPlayers={8}
