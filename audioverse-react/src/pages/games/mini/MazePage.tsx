...
export default function MazePage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'maze')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'treasureHunt', label: t('miniGames.treasureHunt', 'Treasure Hunt') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'mazeSize', label: t('miniGames.mazeSize', 'Maze Size'), type: 'select', options: [
    ...
    { key: 'wallStyle', label: t('miniGames.wallStyle', 'Wall Style'), type: 'select', options: [
      { value: 'classic', label: t('miniGames.classic', 'Classic') },
      { value: 'random', label: t('miniGames.random', 'Random') },
    ], defaultValue: 'classic' },
  ]

  if (phase === 'game') {
    return <MazeGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.maze', 'Maze')}`}
      minPlayers={1}
      maxPlayers={4}
