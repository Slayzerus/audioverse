/**
 * SettlersPage — lobby → settings → title → game flow for the Settlers mini game.
 *
 * Settlers-style indirect-control: place buildings and set priorities,
 * workers do everything automatically.  Supports single / couch / online,
 * VS and co-op modes.
 */
...
export default function SettlersPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'settlers')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      { value: 'build', label: t('miniGames.build', 'Build (Free Play)') },
      { value: 'vs-conquest', label: t('miniGames.vsConquest', 'VS Conquest') },
      { value: 'coop-settlement', label: t('miniGames.coopSettlement', 'Co-op Settlement') },
    ], defaultValue: 'build' },
    ...
    { key: 'startingResources', label: t('miniGames.startingResources', 'Starting Resources'), type: 'select' as const, options: [
  ...
  if (phase === 'game') {
    return <SettlersGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.settlers', 'Settlers')}`}
      minPlayers={1}
      maxPlayers={4}
