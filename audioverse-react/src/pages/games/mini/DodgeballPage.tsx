...
export default function DodgeballPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'dodgeball')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'lastStand', label: t('miniGames.lastStand', 'Last Stand') },
      { value: 'team', label: t('miniGames.team', 'Team') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'ballCount', label: t('miniGames.ballCount', 'Ball Count'), type: 'range', min: 1, max: 8, step: 1, defaultValue: 3 },
    { key: 'ballSpeed', label: t('miniGames.ballSpeed', 'Ball Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
  ]

  if (phase === 'game') {
    return <DodgeballGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.dodgeball', 'Dodgeball')}`}
      minPlayers={1}
      maxPlayers={8}
