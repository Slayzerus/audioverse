...
export default function ArcheryPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'archery')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'movingTargets', label: t('miniGames.movingTargets', 'Moving Targets') },
      { value: 'duel', label: t('miniGames.duel', 'Duel') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'rounds', label: t('miniGames.rounds', 'Rounds'), type: 'range', min: 3, max: 10, step: 1, defaultValue: 5 },
    { key: 'windEnabled', label: t('miniGames.windEnabled', 'Wind Enabled'), type: 'toggle', defaultValue: true },
  ]

  if (phase === 'game') {
    return <ArcheryGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.archery', 'Archery')}`}
      minPlayers={1}
      maxPlayers={4}
