...
export default function ColorMatchPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'color-match')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'speedRound', label: t('miniGames.speedRound', 'Speed Round') },
      { value: 'elimination', label: t('miniGames.elimination', 'Elimination') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'rounds', label: t('miniGames.rounds', 'Rounds'), type: 'range', min: 5, max: 20, step: 1, defaultValue: 10 },
    { key: 'showTime', label: t('miniGames.showTime', 'Show Time (sec)'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
  ]

  if (phase === 'game') {
    return <ColorMatchGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.colorMatch', 'Color Match')}`}
      minPlayers={1}
      maxPlayers={8}
