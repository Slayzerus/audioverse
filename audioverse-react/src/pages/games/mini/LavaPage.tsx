...
export default function LavaPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'lava')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'raceToTop', label: t('miniGames.raceToTop', 'Race to Top') },
      { value: 'lastStanding', label: t('miniGames.lastStanding', 'Last Standing') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'decaySpeed', label: t('miniGames.decaySpeed', 'Decay Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
    { key: 'platformCount', label: t('miniGames.platformCount', 'Platform Count'), type: 'range', min: 5, max: 15, step: 1, defaultValue: 8 },
  ]

  if (phase === 'game') {
    return <LavaGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.lava', 'Lava')}`}
      minPlayers={1}
      maxPlayers={8}
