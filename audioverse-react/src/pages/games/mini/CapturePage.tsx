...
export default function CapturePage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'capture')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'zoneCount', label: t('miniGames.zoneCount', 'Zone Count'), type: 'range', min: 2, max: 6, step: 1, defaultValue: 3 },
    { key: 'roundTime', label: t('miniGames.roundTime', 'Round Time (sec)'), type: 'range', min: 30, max: 120, step: 10, defaultValue: 60 },
  ]

  if (phase === 'game') {
    return <CaptureGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.capture', 'Capture')}`}
      minPlayers={1}
      maxPlayers={8}
