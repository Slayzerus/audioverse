...
export default function BouncePage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'bounce')!
  ...
  if (phase === 'game') {
    return <BounceGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.bounce', 'Bounce')}`}
      minPlayers={1}
      maxPlayers={4}
