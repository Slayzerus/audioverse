/**
 * DoomPage — lobby → settings → title → game flow for the Doom mini game.
 */
...
export default function DoomPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'doom')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      { value: 'campaign', label: t('miniGames.campaign', 'Campaign') },
    ...
    { key: 'levelSize', label: t('miniGames.levelSize', 'Level Size'), type: 'select', options: [
    ...
    { key: 'weaponStart', label: t('miniGames.weaponStart', 'Starting Weapon'), type: 'select', options: [
      { value: 'fist-only', label: t('miniGames.fistOnly', 'Fist Only') },
      { value: 'pistol', label: t('miniGames.pistol', 'Pistol') },
      { value: 'shotgun', label: t('miniGames.shotgun', 'Shotgun') },
    ], defaultValue: 'pistol' },
  ]

  if (phase === 'game') {
    return <DoomGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.doom', 'Doom')}`}
      minPlayers={1}
      maxPlayers={4}
