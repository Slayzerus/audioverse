...
export default function HockeyPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'hockey')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'overtime', label: t('miniGames.overtime', 'Overtime') },
      { value: 'tournament', label: t('miniGames.tournament', 'Tournament') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'scoreToWin', label: t('miniGames.scoreToWin', 'Score to Win'), type: 'range', min: 3, max: 15, step: 1, defaultValue: 7 },
    { key: 'puckSpeed', label: t('miniGames.puckSpeed', 'Puck Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
  ]

  if (phase === 'game') {
    return <HockeyGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.hockey', 'Hockey')}`}
      minPlayers={1}
      maxPlayers={4}
