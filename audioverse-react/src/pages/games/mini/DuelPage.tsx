...
export default function DuelPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'duel')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'bestOfFive', label: t('miniGames.bestOfFive', 'Best of Five') },
      { value: 'quickDraw', label: t('miniGames.quickDraw', 'Quick Draw') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'rounds', label: t('miniGames.rounds', 'Rounds'), type: 'range', min: 3, max: 10, step: 1, defaultValue: 5 },
    { key: 'signalDelay', label: t('miniGames.signalDelay', 'Signal Delay'), type: 'select', options: [
  ...
  if (phase === 'game') {
    return <DuelGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.duel', 'Duel')}`}
      minPlayers={1}
      maxPlayers={8}
