...
export default function SurvivePage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'survive')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'teamSurvive', label: t('miniGames.teamSurvive', 'Team Survive') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'waveSpeed', label: t('miniGames.waveSpeed', 'Wave Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
    { key: 'projectileCount', label: t('miniGames.projectileCount', 'Projectile Count'), type: 'range', min: 3, max: 12, step: 1, defaultValue: 6 },
  ]

  if (phase === 'game') {
    return <SurviveGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.survive', 'Survive')}`}
      minPlayers={1}
      maxPlayers={8}
