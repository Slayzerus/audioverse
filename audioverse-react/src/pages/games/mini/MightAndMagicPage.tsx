/**
 * MightAndMagicPage — lobby → settings → title → game flow for the Might & Magic mini game.
 */
...
import MightAndMagicGame from './MightAndMagicGame'
...
export default function MightAndMagicPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'might-and-magic')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'coop-dungeon', label: t('miniGames.coopDungeon', 'Coop Dungeon') },
    ...
    { key: 'dungeonSize', label: t('miniGames.dungeonSize', 'Dungeon Size'), type: 'select' as const, options: [
  ...
  if (phase === 'game') {
    return <MightAndMagicGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.mightAndMagic', 'Might & Magic')}`}
      minPlayers={1}
      maxPlayers={4}
