/**
 * TetrisPage — lobby → settings → title → game flow for the Tetris mini game.
 */
...
export default function TetrisPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'tetris')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'battle', label: t('miniGames.battle', 'Battle (VS)') },
    ...
    { key: 'startLevel', label: t('miniGames.startLevel', 'Start Level'), type: 'range', min: 1, max: 10, step: 1, defaultValue: 1 },
  ]

  if (phase === 'game') {
    return <TetrisGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.tetris', 'Tetris')}`}
      minPlayers={1}
      maxPlayers={4}
