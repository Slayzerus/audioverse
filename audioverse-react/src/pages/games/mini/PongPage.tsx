...
export default function PongPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'pong')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'multiball', label: t('miniGames.multiball', 'Multiball') },
      { value: 'team', label: t('miniGames.team', 'Team') },
    ], defaultValue: 'classic' },
    DIFFICULTY_SETTING(t),
    { key: 'scoreToWin', label: t('miniGames.scoreToWin', 'Score to Win'), type: 'range', min: 5, max: 21, step: 1, defaultValue: 11 },
    { key: 'ballSpeed', label: t('miniGames.ballSpeed', 'Ball Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
  ]

  if (phase === 'game') {
    return <PongGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.pong', 'Pong')}`}
      minPlayers={1}
      maxPlayers={4}
