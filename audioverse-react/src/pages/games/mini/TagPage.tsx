...
export default function TagPage() {
  ...
  const gameMeta = MINI_GAMES.find(g => g.id === 'tag')!
  ...
  const settingsDefs: SettingDef[] = [
    ...
    { key: 'gameMode', label: t('miniGames.gameMode', 'Game Mode'), type: 'select' as const, options: [
      ...
      { value: 'freezeTag', label: t('miniGames.freezeTag', 'Freeze Tag') },
      { value: 'infection', label: t('miniGames.infection', 'Infection') },
    ...
    { key: 'tagSpeed', label: t('miniGames.tagSpeed', 'Tag Speed'), type: 'range', min: 1, max: 5, step: 1, defaultValue: 3 },
  ]

  if (phase === 'game') {
    return <TagGame players={allPlayers} config={gameConfig} onBack={() => setPhase('lobby')} />
  }
  ...
  return (
    <PlayerLobby
      title={`${gameMeta.icon} ${t('miniGames.tag', 'Tag')}`}
      minPlayers={1}
      maxPlayers={8}
