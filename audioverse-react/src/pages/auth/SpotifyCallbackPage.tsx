...
const SpotifyCallbackPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (error) {
            ...
            setErrorMsg(error === "access_denied"
                ? "Access denied – you declined the Spotify authorization."
        ...
        const redirectUri = `${window.location.origin}/spotifyCallback`;

        postOAuthCallback("spotify", { code, redirectUri, state })
            .then(() => {
                setStatus("success");
            ...
            .catch((err: unknown) => {
                ...
                setErrorMsg(`Failed to connect Spotify: ${message}`);
            });
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", gap: "1rem" }}>
            {status === "processing" && (
                <>
                    <div className="spinner" style={{ width: 48, height: 48, border: "4px solid #ccc", borderTopColor: "#1db954", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
