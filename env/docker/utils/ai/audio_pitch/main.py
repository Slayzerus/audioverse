...
async def pitch_deepf0(file: UploadFile = File(...), model_path: str | None = None):
    ...
    try:
        sess = ort.InferenceSession(cfg_path, providers=['CPUExecutionProvider'])
...
async def pitch_pyin(file: UploadFile = File(...)):
    """Detekcja F0 algorytmem pYIN (probabilistyczny YIN, librosa).
    Parametry: fmin=50 Hz, fmax=1100 Hz, frame_length=2048, sr=16000.
    Zwraca trajektorię {t, hz} i voiced_mask analogicznie do /pitch (CREPE).
    """
    start = time.perf_counter()
    raw = await file.read()
    wav, sr = sf.read(io.BytesIO(raw), dtype='float32')
    ...
    if sr != target_sr:
        ...
        wav = _librosa.resample(wav, orig_sr=sr, target_sr=target_sr)
        sr = target_sr

    import librosa as _librosa
    f0, voiced_flag, voiced_probs = _librosa.pyin(
        wav,
        fmin=50,
        fmax=1100,
        sr=sr,
    ...
    # NaN → 0 (niedetekowane ramki)
    f0_clean = np.nan_to_num(f0, nan=0.0)
    voiced = voiced_flag.astype(int).tolist()
    hop_length = 320
    times = (np.arange(len(f0_clean)) * hop_length / sr).tolist()
    median = float(np.median(f0_clean[f0_clean > 0])) if np.any(f0_clean > 0) else None
    ...
    return JSONResponse({
        "method": "pyin",
        "median_hz": median,
        "track": [{"t": float(t), "hz": float(h)} for t, h in zip(times, f0_clean)],
        "voiced_mask": voiced,
    })
...
async def websocket_pitch_server(ws: WebSocket):
    """Server-side live pitch detection using torchcrepe.
    ...
    Uses a decoupled recv/infer architecture to prevent queue buildup:
      ...
      - infer_loop fires at a fixed rate (≤4/sec) on a 1 s snapshot of freshest audio.
    This prevents the growing latency caused by processing an ever-growing buffer.
    """
    await ws.accept()
    sr          = 16000
    hop_length  = int(sr * 0.01)   # 10 ms
    min_samples = sr // 2           # need ≥0.5 s before first inference
    snap_len    = sr                # analyse last 1 s (fixed window — crepe scales with length)
    ...
    def _run_crepe(b: np.ndarray) -> tuple:
        x = torch.tensor(b, dtype=torch.float32)[None]
        with torch.no_grad():
    ...
    async def infer_loop() -> None:
        nonlocal buf
        while True:
            await asyncio.sleep(0.25)        # ≤ 4 inferences/sec — crepe is slow
            async with buf_lock:
